...
const Repo = ({ repo }) => {
  const formattedCreatedAt = formatDate(repo.created_at);

  const handleCloneClick = async (repo) => {
    try {
      await navigator.clipboard.writeText(repo.clone_url); // Copy the url to clipbard
      toast.success("Repo URL succesfully copied to clipboard");
    } catch (err) {
      toast.err("Failed to copy the url", err);
    }
  };
    ...
    <li className="mb-10 ms-7">
      ...
      <time
        className="block my-1 text-xs font-normal leading-none
			 text-gray-400"
      >
        Released on {formattedCreatedAt}
      </time>
